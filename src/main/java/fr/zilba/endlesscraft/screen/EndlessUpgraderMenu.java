...
public class EndlessUpgraderMenu extends AbstractContainerMenu {
  ...
  public final EndlessUpgraderBlockEntity blockEntity;
  private final Level level;

  public EndlessUpgraderMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
    this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
  }

  public EndlessUpgraderMenu(int pContainerId, Inventory inv, BlockEntity entity) {
    super(ModMenuTypes.ENDLESS_UPGRADER_MENU.get(), pContainerId);
    checkContainerSize(inv, 3);
    blockEntity = (EndlessUpgraderBlockEntity) entity;
    ...
    this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
      ...
      addSlot(new SlotItemHandler(iItemHandler, 2, 98, 48) {
        ...
        public void onTake(Player pPlayer, ItemStack pStack) {
          blockEntity.shrinkIngredientsSlots();
  ...
  public boolean stillValid(Player pPlayer) {
    return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
        pPlayer, ModBlocks.ENDLESS_UPGRADER.get());  }
